...
// 동혁 수정 : 변수 선언과 속성 설정을 Loop 시작 전으로 이동
// 2. Create NetDeviceContainer & Helper Setup
//////// Your Code ////////
...
//////// Your Code ////////
for (int i = 1; i <= 6; ++i) {
    // n0(Hub)와 ni(Spoke) 연결
    NetDeviceContainer link = pointToPoint.Install (nodes.Get(0), nodes.Get(i));
    devices.Add(link);

    // 각 링크마다 다른 서브넷 할당 (10.1.1.0 ~ 10.1.6.0)
    std::ostringstream subnet;
    subnet << "10.1." << i << ".0";
    address.SetBase (subnet.str().c_str(), "255.255.255.0");
    
    // IP 할당 및 저장
    interfaces[i] = address.Assign (link);
}
...
//////// Your Code ////////
  UdpEchoClientHelper echoClient1 (Ipv4Address ("10.1.4.2"), 9);
...
//////// Your Code ////////
  ApplicationContainer serverApps3 = echoServer3.Install (nodes.Get (6));
...
//////// Your Code ////////
  UdpEchoClientHelper echoClient3 (Ipv4Address ("10.1.6.2"), 11);
